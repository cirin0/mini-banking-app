...
async function verifyProxy() {
  console.log('🔍 Перевірка роботи Proxy Pattern...\n');

  const moduleFixture: TestingModule = await Test.createTestingModule({
    imports: [AppModule],
  }).compile();

  const monitoringService = moduleFixture.get<MonitoringService>(
    MonitoringService,
  );
  ...
  // Отримуємо статистику по модулях
  ...
  allStats.forEach((stats) => {
    ...
    console.log(`   - Середній час виконання: ${stats.averageDuration.toFixed(2)}ms`);
    console.log(`   - Частота помилок: ${(stats.errorRate * 100).toFixed(2)}%`);
    console.log(`   - Операції:`);
    Object.entries(stats.operations).forEach(([op, metric]) => {
      console.log(`     • ${op}:`);
      console.log(`       - Викликів: ${metric.count}`);
      console.log(`       - Середній час: ${metric.averageDuration.toFixed(2)}ms`);
  ...
  if (transactionsStats) {
    ...
    const createTransaction = transactionsStats.operations['createTransaction'];
